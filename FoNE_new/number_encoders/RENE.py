...
class RENE(nn.Module):
    def __init__(self, embedding_dim, int_digit_len=5, frac_digit_len=5, period_base_list=[2, 5], add_linear=True, device='cuda'):
        super().__init__()
        print(f'---- \n reduced fourier embedding initing... int_digit_len = {int_digit_len},  frac_digit_len = {frac_digit_len}, period_base_list = {period_base_list}, embedding dim= {embedding_dim}')
        ...
        # Create and register the precomputed cosine matrix as a buffer
        with torch.no_grad():
            precomputed_cos_matrix = self._create_precomputed_cos_matrix(period_base_list).T
        self.register_buffer("precomputed_cos_matrix", precomputed_cos_matrix.to(device))
        # Precompute the cosine frequencies and register as a buffer
        ...
        frequencies = (torch.pi / period_tensor).to(device)  # Changed to pi/T instead of 2*pi/T
    ...
    def forward(self, number_scatter, len_gen=None):
        """Compute reduced Fourier-based embedding with a linear transformation."""
    ...
    def fourier_embedding(self, number_scatter, len_gen=None):
        """Apply reduced Fourier embedding transformation to the input number scatter."""
        if len_gen is None:
            len_gen = 0
        # Flatten the number scatter tensor for processing
        flattened_number_scatter = number_scatter.flatten()
        
        # Transform numbers into cosine-based embeddings
        number_in_hidden_space = self._turn_numbers_to_cosx(flattened_number_scatter, len_gen)
    ...
    def _turn_numbers_to_cosx(self, numbers, len_gen):
        """
        Use precomputed frequencies to turn numbers into cosine embeddings.
        """
        ...
        # Calculate cosine values with broadcasting
        ...
        result[:, :cos_values.shape[1]] = cos_values
        
        return result

    def _create_precomputed_cos_matrix(self, period_base_list=[2, 5]):
        """
        Creates a precomputed cosine matrix for the given period base list and number of positions.
        """
        ...
        cos_list = []
        # Compute cosine values for each period
        for period in period_base_list:
            w = torch.pi / period  # Changed to pi/T instead of 2*pi/T
            cos_list.append(torch.cos(w * base_positions))
        
        # Stack all values to form a matrix
        cos_matrix = torch.stack(cos_list, dim=1)
        return cos_matrix

    def fourier_compute_loss(self, before_decoder, label, int_digit_len, frac_digit_len, len_gen=None):
        """
        Compute loss using cross-entropy for the predicted reduced Fourier embeddings.
        """
        if len_gen is None:
            len_gen = 0
        ...
        num_cos_per_digit = self.precomputed_cos_matrix.shape[0]
        start_idx = self.period_base_list_len*len_gen
        end_idx = self.period_base_list_len*len_gen + (num_digits * num_cos_per_digit)        

        # Reshape to match digits and cosine values
        slices_all_digits = before_decoder[..., start_idx:end_idx]
        slices_all_digits = slices_all_digits.view(-1, num_digits, num_cos_per_digit)

        # Compute logits
        logits_all_digits = torch.matmul(slices_all_digits, self.precomputed_cos_matrix)
    ...
    def fourier_compute_prediction(self, before_decoder, int_digit_len, frac_digit_len):
        ...
        for i in range(frac_digit_len):
            start_idx = i * num_cos_per_digit
            end_idx = start_idx + num_cos_per_digit
            slice_i = before_decoder[..., start_idx:end_idx]
            logits_i = torch.matmul(slice_i, self.precomputed_cos_matrix)
        ...
        for j in range(int_digit_len):
            start_idx = (frac_digit_len + j) * num_cos_per_digit
            end_idx = start_idx + num_cos_per_digit
            slice_j = before_decoder[..., start_idx:end_idx]
            logits_j = torch.matmul(slice_j, self.precomputed_cos_matrix)
...
def main():
    ...
    # Instantiate RENE model
    model = RENE(embedding_dim, int_digit_len, frac_digit_len, period_base_list, device).to(device)
